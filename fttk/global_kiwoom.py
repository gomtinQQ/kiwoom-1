...
class GlobalKiwoom(QAxWidget, QMainWindow):
    def __init__(self):
        ...
        self.maximum_profit_tick = 0
        self.maximum_loss_tick = 0
        self.current_profit_loss_tick = 0 # 실현손익이 +인 상황에서는 0보다 큰 +, -인 상황에서는 0보다 작은 -
    ...
    def start_trade(self):
        ...
        message = f'자동매매 프로그램을 시작합니다. 종목코드: {self.code_symbol}, 기울기: {self.basis_ma}, ' \
                  ...
                  f'최대 익절 틱: {self.maximum_profit_tick}, 최대 손절 틱: {self.maximum_loss_tick},' \
    ...
    def get_current_position(self):
        ...
        self.dynamicCall("CommRqData(QString, QString, QString, QString)", "미결제잔고내역조회", "opw30003", '', "2001")
    ...
    def is_reach_maximum_profit_tick(self, code):
        if 0 < self.maximum_profit_tick <= self.current_profit_loss_tick:
            self.liquidate_position_if_have_position(code)
            message = f'최대 익절 틱에 도달하여 프로그램이 종료됩니다. 익절 틱 개수: {self.current_profit_loss_tick}'
            TelegramBot.send_message(kei_id, message)
            sys.exit()

    def is_reach_maximum_loss_tick(self, code):
        if 0 < self.maximum_loss_tick and (self.current_profit_loss_tick < 0 and abs(self.current_profit_loss_tick) > self.maximum_loss_tick):
            self.liquidate_position_if_have_position(code)
            message = f'최대 손절 틱에 도달하여 프로그램이 종료됩니다. 손절 틱 개수: {self.current_profit_loss_tick}'
            TelegramBot.send_message(kei_id, message)
            sys.exit()
    ...
    def get_tick_data(self, sCode, sRealType, sRealData):
        ...
        self.is_reach_maximum_profit_tick(sCode)
        self.is_reach_maximum_loss_tick(sCode)
    ...
    def calculate_profit_loss_tick(self, code, current_position, next_entry_price):
        current_entry_price = self.get_current_entry_price(code)

        if current_position == 'LONG':
            self.current_profit_loss_tick = self.current_profit_loss_tick + round((next_entry_price - current_entry_price) / float(self.tick_unit[code]))
        else:
            self.current_profit_loss_tick = self.current_profit_loss_tick + round((current_entry_price - next_entry_price) / float(self.tick_unit[code]))

        message = f'현재 실현 손익(틱 기준): {self.current_profit_loss_tick}'
        TelegramBot.send_message(kei_id, message)
    ...
    def trade(self, code, quantity, candle, stop_loss_basis_candles):
        ...
        if ma_diff < 0 and abs(ma_diff) > self.basis_ma and (position == 0 or position == 2):
            if position == 0 and code not in self.stop_loss:
                ...
                self.calculate_profit_loss_tick(code=code, next_entry_price=candle.close_price, current_position='LONG')
            ...
            if position == 0 and code not in self.stop_loss:
                ...
                self.calculate_profit_loss_tick(code=code, next_entry_price=candle.close_price, current_position='SHORT')
        ...
        if code in self.stop_loss:
            ...
            if position == 2: # 2: buy, 1: sell
                if self.stop_loss[code] > current_price:
                    ...
                    self.calculate_profit_loss_tick(code=code, next_entry_price=current_price,
    ...
    def set_maximum_profit_tick(self, maximum_profit_tick):
        self.maximum_profit_tick = maximum_profit_tick

    def set_maximum_loss_tick(self, maximum_loss_tick):
        self.maximum_loss_tick = maximum_loss_tick
